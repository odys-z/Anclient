...
export default class AES2 {
	...
	encrypt(txt: string, key: string, iv: Uint8Array): string {
		let textBytes = this.aesjs.utils.utf8.toBytes(txt);
		// PKCS#7 padding
		const blockSize = 16;
		...
		paddedBytes.fill(padLength, textBytes.length);
		// console.log("textBytes", paddedBytes);

		// var keyBytes = this.aesjs.utils.utf8.toBytes(key);
		let keymd = sha256.array(key) as any;
		// console.log("keymd", keymd, "iv", iv);
		let aesCbc = new this.aesjs.ModeOfOperation.cbc(keymd, iv);
		let encryptedBytes = aesCbc.encrypt(paddedBytes);
		// console.log(encryptedBytes);
		let encryptedB64 = this.bytesToB64(encryptedBytes);
	 ...
	 * </pre>
	 */
	repackSessionToken(ssToken: string, key: string, uid: string): string {
		return this.repackSessionToken_(ssToken, key, uid, this.getIv128());
	}

	repackSessionToken_(ssToken: string, key: string, uid: string, iv_encrypt: Uint8Array): string {
		let ss = ssToken.split(":");

		let plain = this.decrypt(ss[0], key, this.decode64(ss[1]) as unknown as Uint8Array);
		console.log(plain.length, plain);
		// if (plain.length == 32)
		// 	plain = plain.substring(8);
		// console.log(plain.length, plain);

		let cipher = this.encrypt(uid + ":" + plain, key, iv_encrypt);
		console.log(cipher.length, cipher);
		return cipher + ":" + this.encode64(iv_encrypt);
	}
}

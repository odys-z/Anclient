...
/**Case 01.2: parse an AnDatasetResp and convert its resultset into
 * name/value pairs for an AnReact combobox.
 *
 * Exported so it can be invoked directly - e.g. from a browser debug
 * page - without going through mocha's describe/it registration.
 */
export function case01_2_extDataset() {
...
// Guarded: describe/it only exist as globals once mocha.setup() (browser)
// or the mocha CLI/runner (node) has installed them. This lets the module
// be imported on its own - e.g. by a plain debug page that just wants to
// call case01_2_extDataset() directly - without throwing a ReferenceError.
if (typeof describe === 'function') {
	describe('case: [01.2 Ext dataset]', () => {
		it('convert to AnReact bombobox', case01_2_extDataset);
	});
}

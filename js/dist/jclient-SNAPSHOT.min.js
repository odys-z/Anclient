...
class J {
  ...
  respObjs(resp, start, len) {
    ...
    if (typeof len !== 'Number') len = resp.data.rs[0].length - 1;else len = Math.min(len, resp.data.rs[0].length - 1 - start);
    var objs = [];
...
class SessionClient {
  ...
  insert(conn, maintbl, nvs) {
    if (this.currentAct === undefined || this.currentAct.func === undefined) console.error("jclient is designed to support user updating log natively, User action with function Id shouldn't ignored.", "To setup user's action information, call ssClient.usrAct().");
    var upd = new _protocol_js__WEBPACK_IMPORTED_MODULE_2__["InsertReq"](conn, maintbl); // upd.a = Protocol.CRUD.c;
...
class UpdateReq {
  ...
  nv(n, v) {
    if (Array.isArray(n)) this.nvs = this.nvs.concat(Protocol.nvs2row(n));else this.nvs.push([n, v]);
    return this;
  }
  ...
  post(pst) {
    ...
    if (this.postUpds === undefined) this.postUpds = [];
...
class DatasetCfg extends QueryReq {
  ...
  checkt(t) {
    if (t !== ds_t.sqltree && t !== ds_t.retree && t !== ds_t.reforest) console.warn("t won't be understood by server:", t, "Should be one of", ds_t);
  }

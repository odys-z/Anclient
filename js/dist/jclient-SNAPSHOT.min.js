...
class J {
  ...
  respObjs(resp, start, len) {
    ...
    if (typeof start !== 'number') start = 0; // else start++;

    if (typeof len !== 'number') len = resp.data.rs[0].length - 1;else len = Math.min(len, resp.body[0].rs[0].results.length); // var objs = [];

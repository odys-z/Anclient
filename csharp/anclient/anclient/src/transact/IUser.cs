...
namespace io.odysz.semantics
{
	...
	{
		io.odysz.semantics.meta.TableMeta meta();
		...
		System.Collections.Generic.List<string> dbLog(System.Collections.Generic.List<string
			> sqls);
		...
		io.odysz.semantics.IUser logAct(string funcName, string funcId);

		io.odysz.semantics.SemanticObject logout();

		/// <exception cref="System.IO.IOException"/>
		void writeJsonRespValue(object writer);

		// public String sessionId();
		io.odysz.semantics.IUser sessionId(string ssId);

		/// <summary>Add notifyings</summary>
		/// <param name="n"/>
		/// <returns>this</returns>
		/// <exception cref="io.odysz.transact.x.TransException"></exception>
		io.odysz.semantics.IUser notify(object note);

		/// <summary>Get notified string list.</summary>
		/// <returns>notifyings</returns>
		System.Collections.Generic.IList<object> notifies();
	}
}

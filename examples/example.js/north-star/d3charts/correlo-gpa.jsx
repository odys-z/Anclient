...
const tempHeats = [
	...
	['v9', 'Issac', 57], ['v10', 'Issac', 32, 2], ['v4', 'Issac', 20, 5],
...
class CorreloGPAComp extends React.Component {
	...
	componentDidMount() {
		...
		this.tier.gpaCorrelo({uri: this.props.uri}, this.initChart);
	}
	...
	initTest(data) {
		...
		const maxv = d3.max(data, d => +d[2]);
		const maxg = d3.max(data, d => +d[3] || 5);
		...
		const tooltip = d3.select("#correlogram")
			.append("div")
			.style("opacity", 0.)
			.attr("class", "tooltip")
		...
		let r0 = d3.min([x.bandwidth(), y.bandwidth()]) * 0.4;
	...
	initChart(resp) {
		...
		let mousemove = function(e, d) {
			...
			// console.log(width, e.target.cx.baseVal.value);
			tooltip
			  .html(`<h4>${d[1]}/${d[0]}</h4>briefing:<br>Emotion Index: ${d[2]}<br>GPA: ${d[3]}`)
			  // .style("left", x(d[0]) + "px")
			  // .style("top", y(d[1]) - height - margin.top + "px")
			  .style("left", e.target.cx.baseVal.value - 60 + "px")
			  .style("top", e.target.cy.baseVal.value - height + "px")
			  .style("background-color", bg);
		}
	...
	render() {
		...
		// 	this.initChart(this.props.vectors);

		return <div id='correlogram' />;
	}
}
